...
fwvarbytes=$(echo $(nvram -x 8BE4DF61-93CA-11D2-AA0D-00E098032B8C:"Boot$1" | awk '/\<data\>/{flag=1; next} /\<\/data\>/{flag=0} flag') | base64 -d | xxd -c 0 -p)
wbmdevice=$(echo "$fwvarbytes" | tr -d '\n' | sed 's/7fff0400.*/7fff0400/;s/.*04012a/04012a/;s/.\{2\}/&,/g;s/,$//')
wbmoptdata=$(echo "$fwvarbytes" | tr -d '\n' | sed 's/.*57494e444f5753/57494e444f5753/;s/.\{2\}/&,/g;s/,$//')
